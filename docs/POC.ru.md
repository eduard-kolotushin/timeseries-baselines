...
> **Обновление v3.** v3 меняет то, что делает тик, а не сам механизм владения, поэтому все проверки ниже
> остаются в силе. Читайте их вместе с отличиями:
>
> - Тик больше не обучает и не публикует по каждому хэшу. Он сканирует, забирает просроченные перетренировки и
>   публикует из сохранённого снапшота; с `BASELINE_STORE_*` публикация делает **ноль** запросов к Druid и
>   переживает недоступность Druid.
> - Каждый запрос к Druid ограничен по времени и по скорости (`DRUID_MAX_RANGE`, `DRUID_MAX_RPS`,
>   `DRUID_MAX_INFLIGHT`, `DRUID_TIMEOUT`, `DRUID_RETRIES`). Сканирование тоже оконное, поэтому окно
>   допустимости — `SCAN_RANGE` (по умолчанию `max(2*LOOKBACK, 24h)`), а не вся история: хэш, данные которого
>   закончились раньше `now - SCAN_RANGE`, перестаёт быть допустимым.
> - Пригодность не изменилась (`span >= LOOKBACK`), и путь через хранилище применяет её на каждом своём
>   решении: хэш ниже порога не получает строки расписания, не публикуется даже с сохранённым снапшотом, не
>   держит подгонку в памяти, а устаревшая строка для него закрывается ошибкой, а не обучением на остатке окна.
> - Публикуемая метка времени — усечённый до минуты `now` + `AHEAD_MINUTES`, а не последняя точка + N.
> - `SHARD_MEMBERSHIP` (по умолчанию `auto`) выбирает источник участников: `SHARD_PEERS`, затем `SHARD_DNS`,
>   затем heartbeat в Postgres (`store`), затем только сам worker. **`store` — рекомендуемый путь для VM**:
>   набор участников — это строки `baselines.workers`, виденные за `WORKER_TTL` (по умолчанию `max(30s, 2*INTERVAL)`), поэтому
>   добавление и удаление процесса не требует правки списка.
> - `SHARD_PEERS` / `SHARD_DNS` работают ровно так, как описано ниже, включая негативные проверки.
...
| Смена состава теряет не более одного тика | Новый владелец публикует свой горизонт по настенным часам на ближайшем тике; бэкфилла нет |
| Ретрейн не пропускает минуту | Горизонт тика берётся из часов, прочитанных в его начале, поэтому скан и нарезанный ретрейн, вышедшие за минуту, всё равно публикуют её |
| Медленный Druid не останавливает публикацию | С `BASELINE_STORE_*` путь публикации читает только кэш снапшотов: ноль запросов к Druid и ни одного вызова `Series` в установившемся режиме |
...
flowchart LR
  ...
  subgraph w1["worker A"]
    A1["peers = SHARD_PEERS, A-записи DNS или таблица heartbeat, плюс сам себя"] --> A2["Hashes(now-SCAN_RANGE, now), один скан с кэшем на HASH_SCAN_TTL"]
    A2 --> A3["owner(hash) == self ?"]
    A3 -->|"своя доля"| A7["пригодность: span >= LOOKBACK ?"]
    A7 -->|"да"| A4["Restore изменившихся, ForecastRange(ts-1m, ts)"]
    A7 -->|"только пригодные свои хэши"| A5["расписание на пригодный набор, захват просроченных ретрейнов по всему флоту"]
    A7 -->|"нет"| A8["ineligible: попадает в строку тика, не получает ни расписания, ни точки"]
    A5 --> A6["Series(hash, now-LOOKBACK, now) с нарезкой по DRUID_MAX_RANGE"]
  end
  subgraph w2["worker B"]
    B1["тот же набор участников"] --> B2["тот же скан, с кэшем"]
    B2 --> B3["owner(hash) == self ?"]
    B3 -->|"своя доля"| B7["пригодность: span >= LOOKBACK ?"]
    B7 -->|"да"| B4["Restore изменившихся, ForecastRange(ts-1m, ts)"]
    B7 -->|"только пригодные свои хэши"| B5["расписание на пригодный набор, захват просроченных ретрейнов"]
    B7 -->|"нет"| B8["ineligible: попадает в строку тика"]
  end
  H -->|"оконный скан"| A2
  H -->|"оконный ретрейн, только он"| A6
  H -->|"оконный скан"| B2
  A5 -->|"FOR UPDATE SKIP LOCKED"| R
  B5 -->|"FOR UPDATE SKIP LOCKED"| R
  A6 -->|"FitSeasonalBaseline → SnapshotOf → Put"| S
  S -->|"updated_at сдвинулся"| A4
  S -->|"updated_at сдвинулся"| B4
  A4 -->|"now.Truncate(1m) + AHEAD_MINUTES"| K["топик Kafka baselines, свой ключ на точку"]
  B4 -->|"now.Truncate(1m) + AHEAD_MINUTES"| K
```

Порядок тика — скан → владение → пригодность (`span >= LOOKBACK`) → публикация из снапшота: строка
расписания заводится только для пригодных своих хэшей, ретрейн тренирует только их, а свой хэш ниже
`LOOKBACK` не получает ни строки, ни точки и попадает в `ineligible` в строке тика
...
| `DRUID_MAX_RANGE`, `DRUID_MAX_RPS`, `DRUID_MAX_INFLIGHT`, `DRUID_TIMEOUT`, `DRUID_RETRIES` | Границы Druid из v3: максимум времени на один запрос, запросов в секунду, одновременных запросов, таймаут, повторы |
| `HASH_SCAN_TTL`, `SCAN_RANGE` | Кэш сканирования и окно допустимости из v3 |
| `BASELINE_STORE_*` | Postgres-хранилище снапшотов/перетренировок/членства (по полям откатывается на `FORECAST_STORE_*`). Не задано: fit и публикация каждый тик, поведение v1/v2 |
| `DEFAULT_RETRAIN_CRON`, `RETRAIN_RETRY`, `TRAIN_CONCURRENCY`, `SNAPSHOT_CACHE_TTL` | Расписание перетренировок, задержка повтора/лиз, одновременные перетренировки, проверка свежести снапшота |
| `SHARD_MEMBERSHIP` | `auto` (по умолчанию), `peers`, `dns` или `store` |
| `WORKER_TTL` | Строка `baselines.workers` считается участником, пока её видели не позже этого срока (по умолчанию `max(30s, 2*INTERVAL)` и всегда больше `INTERVAL`) |
...
Пустые `SHARD_PEERS`, `SHARD_DNS` и хранилище означают, что все хэши принадлежат одному worker'у. `SHARD_PEERS`
и `SHARD_DNS` одновременно — ошибка на старте.
...
- **Скан готовности не шардируется.** Каждый worker выполняет `GROUP BY metric_hash` по всей
  таблице, то есть N worker'ов дают N сканов; делятся только `Series`-загрузка и подгонка по своим хэшам.
  В v3 скан оконный (`SCAN_RANGE`, нарезается по `DRUID_MAX_RANGE`) и кэшируется на `HASH_SCAN_TTL`, поэтому
  он стоит `N × ceil(SCAN_RANGE / DRUID_MAX_RANGE)` запросов на `HASH_SCAN_TTL`, а не по одному на тик: при
  `SCAN_RANGE=max(2*LOOKBACK,24h)` и `LOOKBACK=336h` это 28 запросов на worker за 5 минут.
...
- **Идентификатор должен быть уникальным.** Процессы на одной машине и любые ручные переопределения не
  должны совпадать, иначе они дважды публикуют одну долю (безвредно, но расточительно — измерено на 2048
  хэшах: 985 опубликованы дважды, ни один не потерян). Потеря доли — отдельный сбой: для него нужен
  участник из списка без процесса (Шаг 1), а не совпадение идентификаторов.
