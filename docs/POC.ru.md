...
flowchart LR
  ...
  subgraph w1["worker A"]
    ...
    A3 -->|"своя доля"| A4["Restore изменившихся, ForecastRange(ts-1m, ts)"]
    A2 --> A5["расписание на свой набор, захват просроченных ретрейнов по всему флоту"]
  ...
  subgraph w2["worker B"]
    ...
    B3 -->|"своя доля"| B4["Restore изменившихся, ForecastRange(ts-1m, ts)"]
    B2 --> B5["расписание на свой набор, захват просроченных ретрейнов"]
  end
  H -->|"оконный скан"| A2
