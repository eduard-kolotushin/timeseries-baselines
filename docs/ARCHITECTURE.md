...
- A `metric_value` that is `null` or unparseable becomes `NaN` — the library's missing-value marker, which the fit drops — and **keeps its timestamp**, because the 1-minute check runs on this series before the fit drops NaN: dropping the point instead would make a gapped series look like a bad step and take the hash out of eligibility. A row whose `__time` cannot be read is dropped and counted in a warning (`druid series rows unusable`), as is an unreadable scan row (`druid hash rows unusable`): a value the worker cannot use is never silently a zero.
